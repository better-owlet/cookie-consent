...
export default class Interface extends Base {
  ...
  buildStyle() {
    return el(
      ...
      '#cconsent-bar { background-color:' +
        ...
        '; color:' +
        this.options.theme.barTextColor +
      ...
      '#cconsent-bar a { text-decoration:underline; color:' + this.options.theme.barTextColor + '; }',
      '#cconsent-bar button { line-height:normal; font-size:14px; border:none; padding:10px 10px; color:' +
        this.options.theme.barMainButtonTextColor +
        '; background-color:' +
        this.options.theme.barMainButtonColor +
        ';}',
      ...
      '#cconsent-modal .ccm__footer button { line-height:normal; font-size:14px; transition: background-color .5s ease-out; background-color:' +
        this.options.theme.modalMainButtonColor +
        '; color:' +
        this.options.theme.modalMainButtonTextColor +
        '; border:none; padding:13px; min-width:110px; border-radius: 2px; cursor:pointer; }',
      '#cconsent-modal .ccm__footer button:hover { opacity: 0.8; }',
  ...
  buildBar() {
    return el(
      ...
      el(
        ...
        el(
          'div.ccb__left',
          el('div.cc-text', Language.getTranslation(this.options, this.options.language.current, 'barMainText')),
        ),
        el(
          'div.ccb__right',
          el(
            'div.ccb__button',
            el('a.ccb__edit', Language.getTranslation(this.options, this.options.language.current, 'barLinkSetting')),
            el(
              'button.consent-give',
              Language.getTranslation(this.options, this.options.language.current, 'barBtnAcceptAll'),
            ),
          ),
        ),
      ),
  ...
  modalRedrawIcons() {
    ...
    for (let tabGroup of Array.prototype.slice.call(tabGroups)) {
      if (this.options.categories[tabGroup.dataset.category].checked) {
  ...
  render(name: string, elem: any, callback: (e: any) => void = () => {}) {
    if (typeof callback === 'undefined') callback = function () {};
    if (typeof this.elements[name] !== 'undefined') {
      this.elements[name] &&
        this.elements[name].parentNode &&
  ...
  addEventListeners() {
    ...
    this.elements['modal']
      ...
      .addEventListener(
        'click',
        (event: {
          target: {classList: {contains: (arg0: string) => any}; dataset: {category: string}; checked: boolean};
        }) => {
          // If you click trough the tabs on Cookie settings
          if (
            (event && event.target && event.target.classList && event.target.classList.contains('ccm__tab-head')) ||
            event.target.classList.contains('ccm__tab-head__icon-wedge')
          ) {
            const getDlParent = (eventTarget: any): any => {
          ...
          if (event.target.classList.contains('category-onoff')) {
            this.options.categories[event.target.dataset.category].wanted = this.options.categories[
              event.target.dataset.category
            ].checked = event.target.checked === true ? true : false;

            var dt = document.querySelector('.ccm__tabgroup.' + event.target.dataset.category);
            if (event.target.checked === false && dt && dt.classList.contains('checked-5jhk')) {
              dt.classList.remove('checked-5jhk');
            } else {
              dt && dt.classList.add('checked-5jhk');
            }
          }
        },
      );
    ...
    const modalSubmit = document.getElementById('ccm__footer__consent-modal-submit');
    modalSubmit &&
      modalSubmit.addEventListener('click', () => {
        ...
        Array.prototype.forEach.call(switchElements, switchElement => {
          this.options.categories[switchElement.dataset.category].wanted = switchElement.checked;
        });
  ...
  writeBufferToDOM() {
    for (let action of this.buffer.appendChild) {
      if (this.options.categories[action.category].wanted === true) {
        Node.prototype.appendChild.apply(action.this, action.arguments);
      }
    }

    for (let action of this.buffer.insertBefore) {
      if (this.options.categories[action.category].wanted === true) {
  ...
  buildCookie(callback: {(cookie: any): void; (arg0: {version: number; categories: {}; services: never[]}): void}) {
    let cookie: {
      version: number;
      categories: {
        [x: string]: any;
    ...
    for (let key in this.options.categories) {
      cookie.categories[key] = {
        wanted: this.options.categories[key].wanted,
      };
    }
  ...
  setCookie(cookie: any, callback?: () => void) {
